...
public class CommonFieldDataResourceIT {
    ...
    @Test
    @Transactional
    void getAllCommonFieldDataBySiteConfigIsEqualToSomething() throws Exception {
        SiteConfig siteConfig = SiteConfigResourceIT.createEntity();
        commonFieldData.setSiteConfig(siteConfig);
        commonFieldDataRepository.insert(commonFieldData);
        Long siteConfigId = siteConfig.getId();
        // Get all the commonFieldDataList where siteConfig equals to siteConfigId
        defaultCommonFieldDataShouldBeFound("siteConfigId.equals=" + siteConfigId);

        // Get all the commonFieldDataList where siteConfig equals to (siteConfigId + 1)
        defaultCommonFieldDataShouldNotBeFound("siteConfigId.equals=" + (siteConfigId + 1));
    }

    @Test
    @Transactional
    void getAllCommonFieldDataByDictionaryIsEqualToSomething() throws Exception {
        Dictionary dictionary = DictionaryResourceIT.createEntity();
        commonFieldData.setDictionary(dictionary);
        commonFieldDataRepository.insert(commonFieldData);
        Long dictionaryId = dictionary.getId();
        // Get all the commonFieldDataList where dictionary equals to dictionaryId
        defaultCommonFieldDataShouldBeFound("dictionaryId.equals=" + dictionaryId);

        // Get all the commonFieldDataList where dictionary equals to (dictionaryId + 1)
        defaultCommonFieldDataShouldNotBeFound("dictionaryId.equals=" + (dictionaryId + 1));
    }
