...
public class CommonFieldDataQueryService implements QueryService<CommonFieldData> {
    ...
    private QueryWrapper<CommonFieldData> createQueryWrapper(
        ...
        if (criteria != null) {
            ...
            if (criteria.getSiteConfigId() != null) {
                queryWrapper =
                    CriteriaUtil.build(useOr, queryWrapper, buildRangeSpecification(criteria.getSiteConfigId(), "site_config_id", useOr));
            }
            if (criteria.getDictionaryId() != null) {
                queryWrapper =
                    CriteriaUtil.build(useOr, queryWrapper, buildRangeSpecification(criteria.getDictionaryId(), "dictionary_id", useOr));
            }
