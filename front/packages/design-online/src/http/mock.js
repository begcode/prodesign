...
export default [
  ...
  // 根据分组ID获取当前分组可以添加的区块
  {
    ...
    response: async config => {
      const url = config.url;
      const groupId =
        url.indexOf('?') > -1 ? url.substring(url.lastIndexOf('/') + 1, url.indexOf('?')) : url.substr(url.lastIndexOf('/') + 1);
      const query = url.substr(url.indexOf('?'));
      const params = new URLSearchParams(query);
      ...
      // 标签过滤结果
      const tagResult = tag && tag.length ? searchResult.filter(item => item.tags?.some(i => tag.includes(i))) : searchResult;
      // 组织过滤结果
      const tenantResult = tenant && tenant.length ? tagResult.filter(item => tenant.includes(String(item.tenant?.id))) : tagResult;
      // 作者过滤结果
      const authorResult = author && author.length ? tenantResult.filter(item => author.includes(String(item.author?.id))) : tenantResult;
