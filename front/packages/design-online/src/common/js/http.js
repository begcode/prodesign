...
import { usePage, useCanvas, useNotify } from '@/controller/index';
 ...
 * @returns { Promise }
 */
export const requestEvent = params => http.post('/platform-center/api/platform/monitoring/event', params).catch(() => {});
...
export const handlePageUpdate = (pageId, params, routerChange) => {
  return http
    .post(`http://localhost:3290/app-center/api/pages/update/${pageId}`, params)
    .then(res => {
      ...
      useNotify({ title: '保存成功!', type: 'success' });
