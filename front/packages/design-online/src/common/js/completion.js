...
import { useCanvas, useResource } from '@/controller/index';
...
const getUserWords = () => {
  ...
  return {
    ...
    stores: {
      ...
      data: globalState
        .filter(item => item.id)
        .map(item => [item.id, ...[...Object.keys(item.state), ...Object.keys(item.getters)].map(name => `${item.id}.${name}`)])
        .flat(),
    },
    storeFn: {
      type: TYPES.Method,
...
const getWords = (model, position) => {
  ...
  const currentWord = model.getWordUntilPosition(position).word ? model.getWordAtPosition(position) : getCurrentChar(model, position);
...
export const initCompletion = monacoInstance => {
  ...
  return ['javascript', 'typescript'].map(lang => monacoInstance.languages.registerCompletionItemProvider(lang, completionItemProvider));
};
