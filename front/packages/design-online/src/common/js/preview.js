...
import { useResource } from '@/controller/index';
...
const open = (params = {}) => {
  ...
  params.scripts = scripts.filter(item => item.script).reduce((pre, cur) => ({ ...pre, [cur.package]: cur.script }), {});
  ...
  openUrl = isDevelopEnv ? `./preview.html?tenant=${tenant}#${hashString}` : `${href}/preview?tenant=${tenant}#${hashString}`;
