  ...
  <div>
    <div :class="['toolbar', { _full: fullscreen }]">
      ...
      <div class="buttons">
        ...
        <tiny-tooltip v-if="showFormatBtn && options.language === 'json'" title="格式化" placement="top">
          <public-icon name="json" @click="formatCode"></public-icon>
        </tiny-tooltip>
        <span v-if="showFullScreenBtn">
          <tiny-tooltip v-if="!fullscreen" title="全屏" placement="top">
            <public-icon name="full-screen" @click="fullscreen = true"></public-icon>
          </tiny-tooltip>
          <tiny-tooltip v-else title="退出全屏" placement="top">
