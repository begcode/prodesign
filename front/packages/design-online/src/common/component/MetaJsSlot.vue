<template>
  <div class="meta-slot-container">
    ...
    <Form ref="slotRef" class="slot-form" :model="slotList" label-width="0">
      <div v-for="(slot, index) in slotList" :key="slot.name" class="use-slot">
        ...
        <div class="use-slot-item-content">
          <div class="use-slot-switch-wrap">
            ...
            <Tooltip effect="dark" :title="state.currentComponent?.content" placement="top">
          ...
          >
            <Input v-model="slot.params" class="use-slot-params" @change="validParmas(slot, paramsPropPath(index))"></Input>
          </FormItem>
        </div>
      </div>
    </Form>
...
import { Input, Tooltip, Form, FormItem } from 'ant-design-vue';
import { useProperties, useCanvas, useModal } from '@/controller';
