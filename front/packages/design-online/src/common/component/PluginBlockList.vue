<template>
  ...
  >
    ...
    >
      <slot :data="item">
        <img v-if="item.screenshot" class="item-image" :src="item.screenshot" draggable="false" @error="$event.target.src = defaultImg" />
        <Icon v-else class="item-image item-default-img" icon="block-default-img|svg"></Icon>
        <div class="item-text">
        ...
        <div v-if="item.isShowProgress" class="progress-bar">
          <tiny-progress :text-inside="true" :stroke-width="8" :percentage="item.publishProgress" status="success"></tiny-progress>
        </div>
        ...
        <div v-else-if="showSettingIcon" :class="['block-setting', { 'is-current-visible-icon': state.hoverItemId === item.id }]" title=" ">
          <tiny-popover
    ...
    <div v-if="showBlockShot && state.hover && state.currentBlock.screenshot" class="block-shortcut">
      ...
      <div class="block-shortcut-image-wrapper">
        <img class="block-shortcut-image" :src="state.currentBlock.screenshot || defaultImg" @error="$event.target.src = defaultImg" />
      </div>
    </div>
  </ul>
</template>
...
import { Tooltip, Progress, Popover } from 'ant-design-vue';
