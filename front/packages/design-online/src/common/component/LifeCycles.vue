<template>
  <div class="life-cycle">
    <Popover v-model:open="state.showPopover" placement="leftBottom" trigger="hover" popperClass="option-popper">
      <template #content>
  ...
  <Modal v-model:open="state.showLifeCyclesDialog" fullscreen :title="state.title" :append-to-body="true">
    ...
    <template #footer>
      <div class="bind-dialog-footer">
        <Button @click="state.showLifeCyclesDialog = false">取 消</Button>
        <Button type="info" @click="editorConfirm">确 定</Button>
      </div>
    </template>
  </Modal>
</template>
...
import { Button, Modal, Popover, InputSearch, Alert } from 'ant-design-vue';
import { getGlobalConfig, useModal, usePage, useNotify } from '@/controller';
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const openLifeCyclesPanel = item => {
      ...
      state.editorValue =
        bindLifeCycleSource?.value || `function ${item} (${item === 'setup' ? '{ props, state, watch, onMounted }' : ''}) {} `;
