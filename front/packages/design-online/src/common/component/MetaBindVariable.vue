<template>
  <slot>
    <span :class="['icon', { 'icon-bind': modelValue?.variable }]" @click="open">
      <svg-button name="cloud-shell|svg" placement="top" tips="变量绑定"></svg-button>
    </span>
  </slot>
  ...
  <Modal
    ...
    @update:open="state.isVisible = $event"
    @close="cancel"
  >
        ...
        <div class="content-left">
          ...
          <div class="list-wrap">
            ...
            <div class="item-content">
              <InputSearch v-model="state.value" placeholder="搜索"></InputSearch>
        ...
        <div class="content-right">
                ...
                <template #content>
                  <div class="bottom lowcode-scrollbar-thin">
                    <div class="bottom-demo">
                      <p>你可以通过点击左侧区域绑定变量或处理函数，或者点击右边的铅笔按钮切换到输入模式，输入复杂的表达式。</p>
          ...
          <div class="content-wrap">
            ...
            <div v-else :class="['top', { 'poll-top': isDataSource }]">
              <monaco-editor ref="editor" :value="state.variable" :options="editorOptions" @editorDidMount="editorDidMount"></monaco-editor>
              <div v-if="isDataSource" class="datasource-poll-wrap">
                <Tooltip placement="top" content="定时更新开启后，页面运行时将会定期请求远程数据源，实现数据定时更新。"
                  ><span>定时更新：</span></Tooltip
                >
                <Switch v-model="state.isPoll"></Switch>
                <div v-if="state.isPoll" class="datasource-poll-interval">
                  <span>更新时间：</span>
                  <Input type="number" v-model="state.pollInterval"></Input>
                  <span>ms</span>
                </div>
              </div>
            </div>
    ...
    <template #footer>
      <div class="bind-dialog-footer">
        <div class="left">
          <Button type="danger" plain @click="remove">移除绑定</Button>
        </div>
        <div class="right">
...
import { Button, Modal, InputSearch, Switch, Input, Tooltip, Alert, Popover } from 'ant-design-vue';
import { useHttp } from '@/http';
import { getSchema, getGlobalState, setState, getNode, getCurrent } from '@/canvas';
import { useCanvas, useResource, useLayout, useApp, useProperties, useData } from '@/controller';
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const editorDidMount = () => {
      ...
      const diagnosticsOptions = editor.value.getMonaco().languages.typescript.javascriptDefaults.getDiagnosticsOptions();
    ...
    const selectItem = item => {
      ...
      if (item.id === 'function') {
        ...
        http.get(`http://localhost:3290/app-center/api/sources/list/${selectedId}`).then(data => {
          const sourceData = {};
          data.forEach(res => {
        ...
        state.bindPrefix = '';
        const [loopItem = DEFAULT_LOOP_NAME.ITEM, loopIndex = DEFAULT_LOOP_NAME.INDEX] = useProperties().getSchema()?.loopArgs || [];
