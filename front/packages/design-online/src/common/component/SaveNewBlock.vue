<template>
  <div>
    <Modal :append-to-body="true" :open="visible" title="新建区块" width="500" @close="cancel">
      <tiny-form
        show-message
        :model="formData"
        :rules="rules"
      ...
      >
        ...
        <tiny-form-item label="请选择区块分类" prop="group">
          <tiny-select v-model="formData.group" :options="categoryList" placeholder="请选择" @change="changeCategory"> </tiny-select>
        </tiny-form-item>
        <p v-show="fromCanvas" class="block-tip">注意: 当前动作拷贝的methods方法只是一个空方法，具体的业务逻辑需要在空方法中自行添加！！</p>
      </tiny-form>
      <template #footer>
        <Button type="primary" @click="addBlock">确定</Button>
...
import { Input, Form, FormItem, Button, Modal, Select } from 'ant-design-vue';
import { useBlock, useLayout, useCanvas, useModal } from '@/controller';
