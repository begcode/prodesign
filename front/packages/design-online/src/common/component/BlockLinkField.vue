    ...
    <template #content>
      <ul class="context-menu">
        ...
        <li v-for="item in properties" :key="item.property" class="menu-item">
          {{ item.property }}
          <span v-if="item.property !== data?.linked?.blockProperty" class="link-item" @click="editProperty(item)"> 关联 </span>
        </li>
      </ul>
    </template>
...
import { Input as TinyInput, Popover as TinyPopover } from 'ant-design-vue';
import { useLayout, useModal, useCanvas, useBlock, useHistory } from '@/controller';
import { getSchema, getCurrent } from '@/canvas';
