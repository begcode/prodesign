...
import { useProperties, useResource, useModal } from '@/controller';
        ...
        useModal().confirm({
          ...
          exec: () => {
            ...
            const nodeIdex = children.findIndex(
              ({ componentName, props }) => componentName === 'Template' && (props?.slot === name || props?.slot?.name === name),
            );
