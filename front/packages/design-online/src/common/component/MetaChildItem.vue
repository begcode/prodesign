<template>
  <div class="items-container">
    <div v-for="(data, idx) in properties" :key="idx" class="meta-config-item">
      <config-item :key="idx" :property="data" :data-prop-index="idx" @update:modelValue="onValueChange(data.property, $event)">
    ...
    const properties = computed(() => {
      ...
      if (result.length && propsModelValue) {
        result.forEach(item => {
          ...
          item.widget.props.modelValue = typeof model_value_property === 'boolean' ? model_value_property : model_value_property || null;
        });
      }
      return result;
    });
