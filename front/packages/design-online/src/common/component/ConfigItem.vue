      ...
      <div class="item-input">
        ...
        >
          ...
          <div v-if="showErrorPopup" class="error-tips-container">
            <Icon icon="notify-failure|svg" class="error-icon"></Icon>
...
import { useHistory, useProperties, useResource, useLayout, useCanvas } from '@/controller';
import { generateFunction } from '@/controller/utils';
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const propLabel = computed(() => props.property.property || props.property?.label?.text?.[locale.value] || props.property?.label?.text);
    ...
    const showCodeEditIcon = computed(
      () => props.isTopLayer && isBindingState.value === false && (multiType.value || ['array', 'object'].includes(props.property.type)),
    );
    const showLabel = computed(
      () =>
        !props.onlyEdit &&
        propLabel.value &&
        (isBindingState.value || !['MetaGroupItem', 'MetaArrayItem', 'MetaRelatedColumns'].includes(widget.value.component)) &&
        !multiType.value,
    );
    ...
    const component = computed(() =>
      multiType.value
        ? MultiTypeSelector
        : props.metaComponents[widget.value.component] || MetaComponents[widget.value.component] || MetaComponents['MetaInput'],
    );
    ...
    const updateValue = value => {
      ...
      if (property === 'children') {
        ...
        if (!useCanvas().isSaved() && ![PAGE_STATUS.Guest, PAGE_STATUS.Occupy].includes(useLayout().layoutState.pageStatus.state)) {
          return;
        }
