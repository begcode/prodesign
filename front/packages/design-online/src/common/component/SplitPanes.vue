...
export default {
  ...
  methods: {
    ...
    calculatePanesSize(drag) {
      ...
      const dragLowCodePercentage = Math.max(Math.min(this.getCurrentDragPercentage(drag), maxLowCodeDrag), minLowCodeDrag);
      ...
      if (paneLowCodeAfter !== null) {
        paneLowCodeAfter.size = Math.min(
          Math.max(100 - dragLowCodePercentage - sums.nextPanesSize - sums.nextLowCodeReachedMinPanes, paneLowCodeAfter.min),
      ...
      if (dragLowCodePercentage > 100 - sums.nextPanesSize - this.panes[panesLowCodeToResize[1]].min) {
        ...
        if (panesLowCodeToResize[1] === undefined) {
          ...
          this.panes[panesLowCodeToResize[0]].size =
            100 - sums.prevPanesSize - sums.nextLowCodeReachedMinPanes - this.panes[this.panesCount - 1].min - sums.nextPanesSize;
          return null;
        }
      }
    ...
    initialPanesSizing() {
      ...
      if (leftLowCodeToAllocate > 0.1) {
        ...
        if (leftLowCodeToAllocate2 > 0.1) this.readjustSizes(leftLowCodeToAllocate, ungrowableLowCode, unshrinkableLowCode);
      }
    },
    ...
    readjustSizes(leftLowCodeToAllocate, ungrowableLowCode, unshrinkableLowCode) {
      ...
      if (leftLowCodeToAllocate > 0) equalLowCodeSpaceToAllocate = leftLowCodeToAllocate / (this.panesCount - ungrowableLowCode.length);
