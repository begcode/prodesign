...
async function formatScript(string) {
  return await prettier.format(string, {
    ...prettierCommon,
    parser: 'babel',
    plugins: [estree, parserBabel],
  });
}
...
async function generateStores({ globalState }) {
  ...
  return [
    {
      ...
      fileContent: await formatScript(result),
    },
  ];
}

async function generatePageFiles(codeList, pagePath = '') {
  const pageFiles = [];

  const formatTypePluginMap = {
    babel: [estree, parserBabel],
    vue: [estree, parserHtml, parserBabel, parseCss],
    javascript: [estree, parserBabel],
    css: [estree, parseCss],
    less: [estree, parseCss],
    sass: [estree, parseCss],
    json: [estree, parserBabel],
  };

  const blockList = codeList.filter(item => item.type === 'Block').map(item => item.panelName);
  for (let i = 0; i < codeList.length; i++) {
    const { panelName, panelValue, prettierOpts, type } = codeList[i];
    console.log('prettierOpts', prettierOpts);
    if (panelName && panelValue) {
      if (prettierOpts?.parser && formatTypePluginMap[prettierOpts.parser]) {
        codeList[i].panelValue = prettier.format(panelValue, {
          ...prettierOpts,
      ...
      if (type === 'Page' && blockList.length) {
        blockList.forEach(blockName => {
          codeList[i].panelValue = codeList[i].panelValue.replace(
            `./${blockName}`,
            `${basePaths.blocks.replace('src/', '@/')}${blockName}`,
          );
        });
      }
      pageFiles.push({
...
async function generateUtils({ utils }) {
  const result = [];

  if (utils?.length) {
    const utilStr = generateExport(utils);
    const content = await formatScript(utilStr);

    result.push({
      fileType: FILE_TYPES.Utils,
      filePath: basePaths.utils,
      fileContent: await formatScript(content),
    });
  }
...
export async function generateRouter(pages) {
  if (!pages?.length) {
    return [];
  }
  ...
  `;
  const codeStr = await formatScript(content);
...
export async function generateVuePage({ params, codeList, metaData, pageList }) {
  const pageFiles = await generatePageFiles(codeList, getCurrentPagePath(params, pageList));
  console.log('params', params);
  console.log('codeList', codeList);
  console.log('metaData', metaData);
  console.log('pageList', pageList);
  const i18nFiles = generateI18n(metaData);
  const utilsFiles = await generateUtils(metaData);
  const dataSourceFiles = generateDataSource(metaData);
  const storeFiles = await generateStores(metaData);
  const routeFiles = await generateRouter(pageList);
