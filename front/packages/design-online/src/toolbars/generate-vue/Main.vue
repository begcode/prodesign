...
import { getGlobalConfig, useBlock, useCanvas, useNotify, useLayout } from '@/controller';
import { fs } from '@/utils';
import { getSchema } from '@/canvas';
import { generateVueBlock, generateVuePage } from './generateCode';
...
export default {
  ...
  props: {
    icon: {
      type: String,
      default: 'ant-design:code-sandbox-outlined',
    },
  },
  setup() {
    ...
    const getToSaveFilesInfo = async ({ params, codeList, metaData, pageList }) => {
      const handlers = {
        Block: generateVueBlock,
        Page: generateVuePage,
      };
      return await handlers[params.type]({ params, codeList, metaData, pageList });
    };
    ...
    const generate = async () => {
      ...
      if (isEmptyPage()) {
        useNotify({ type: 'warning', title: '请先创建页面' });
        return;
      }
      ...
      try {
        ...
        // 暂存待生成代码文件信息
        state.saveFilesInfo = await getToSaveFilesInfo({ params, codeList, metaData, pageList });
