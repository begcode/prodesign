<template>
  <div class="top-panel-logo">
    <div class="logo-wrap" @click.stop="handleTitleClick">
    ...
    <Modal v-model:open="state.show" :title="state.title" width="400px" :append-to-body="true">
      <Form
        ref="form"
        ...
        label-width="110px"
      >
        <FormItem v-if="!state.showPreview" name="version" label="历史版本">
          <Input v-model="state.formData.version" placeholder="请输入版本号"></Input>
        </FormItem>
        <div v-else>
          <FormItem name="commitMsg" label="提交日志">
            <Input v-model="state.formData.commitMsg" placeholder="请输入提交到git库的日志" @blur="repalceTrim"></Input>
          </FormItem>
          <FormItem name="branch" label="分支名称">
            <Input v-model="state.formData.branch" placeholder="请输入分支名称"></Input>
          </FormItem>

          <FormItem name="canCreateNewBranch">
            <template #label>创建分支 </template>

            <Switch v-model="state.formData.canCreateNewBranch">
              <template #open>
                <span>是</span>
              </template>
              <template #close>
          ...
          <FormItem name="allGenerate">
            <template #label>生成工程配置 </template>

            <Switch v-model="state.formData.allGenerate">
              <template #open>
                <span>是</span>
              </template>
              <template #close>
            ...
            <Tooltip
              ...
              popper-class="help-tooltip"
            >
              <icon-help class="tiny-svg-size icon-help" />
            </Tooltip>
          </FormItem>
        </div>
      </Form>
    ...
    <Modal v-model:open="tipBoxVisibility" title="消息" width="30%" :modal="false">
      <span>{{ tipText }}</span>
...
import { Modal, Input, Button as TinyButton, Form, FormItem, Spin, Switch, Tooltip } from 'ant-design-vue';
import { iconHelpCircle } from '@opentiny/vue-icon';
import { useLayout, useApp, getGlobalConfig, useModal } from '@/controller';
...
const form = ref(null);
const menus = ref(getGlobalConfig()?.dslMode === 'Angular' ? [] : [{ name: '应用发布', code: 'publishApp', icon: 'news' }]);
...
const actions = {
  ...
  publishApp() {
    state.show = true;
    const data = localStorage.getItem('tinyengine_publishMsg') ? JSON.parse(localStorage.getItem('tinyengine_publishMsg')) : '';
    if (data) {
