  ...
  <plugin-setting v-if="isShow" :title="state.title" class="pageFolder-plugin-setting">
    <template #header>
      <button-group>
        ...
        <svg-button v-if="!pageSettingState.isNew" name="delete|svg" placement="bottom" tips="删除" @click="deleteFolder"></svg-button>
        <svg-button class="close-plugin-setting-icon" name="close|svg" @click="closeFolderSetting"></svg-button>
      </button-group>
    </template>

    <template #content>
      <div class="page-setting-content">
        <Collapse v-model:activeKey="state.activeName" :bordered="false">
          <CollapsePanel header="基本设置" key="folderGeneralRef">
...
import { PluginSetting, SvgButton, ButtonGroup } from '@/common';
import { usePage, useModal, useApp, useNotify } from '@/controller';
