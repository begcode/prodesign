<template>
  <div class="general-config">
    ...
    >
      <FormItem v-if="!isFolder" name="group" label="页面类型" class="form-item-page-type">
        <RadioGroup v-model:value="pageSettingState.currentPageData.group">
          <Radio :value="'staticPages'">静态页面</Radio>
          <Radio :value="'publicPages'">公共页面</Radio>
        </RadioGroup>
      </FormItem>
      <FormItem name="name" :label="`${isFolder ? '文件夹' : '页面'}ID`">
        <Input v-model:value="pageSettingState.currentPageData.name" :placeholder="`请设置${isFolder ? '文件夹' : '页面'}ID`"></Input>
      </FormItem>

      <FormItem v-if="pageSettingState.currentPageData.group !== 'publicPages'" label="选择父文件夹" name="parentId">
        <tiny-select
      ...
      <FormItem label="路由" name="route">
        <Input v-model:value="pageSettingState.currentPageData.route" placeholder="请设置路由">
          <template #addonBefore><span class="input-head">website.com</span></template>
        </Input>
        <div class="tip">
          <svg-button class="icon" name="text-page-link|svg"></svg-button>
    ...
    <page-home v-if="!isFolder && !pageSettingState.isNew && pageSettingState.currentPageData.group !== 'public'"></page-home>
  </div>
</template>

<script lang="jsx">
import { ref, computed, watchEffect } from 'vue';
import { Form, FormItem, Input, Select, Radio, RadioGroup } from 'ant-design-vue';
import { usePage } from '@/controller';
import { REGEXP_PAGE_NAME, REGEXP_FOLDER_NAME, REGEXP_ROUTE } from '@/common/js/verification';
import { SvgButton } from '@/common';
...
export default {
  ...
  setup() {
        ...
        renderContent: (_h, { node, data }) => {
          return (
            <span style="display:flex">
              {node.isLeaf && data.id !== ROOT_ID ? <svg-icon name="text-page-folder-closed" style="margin-right:14px"></svg-icon> : null}
    ...
    const validGeneralForm = () => {
      return generalForm.value.validate();
    };
