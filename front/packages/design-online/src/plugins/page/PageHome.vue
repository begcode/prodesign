...
import { usePage, useModal, useNotify } from '@/controller';
import { isVsCodeEnv } from '@/common/js/environments';
import { generateRouter } from '@/common/js/vscodeGenerateFile';
        ...
        exec: () => {
          ...
          handleRouteHomeUpdate(id, params)
            .then(() => {
              ...
              useNotify({ title: '主页设置成功！', type: 'success' });
            })
            .catch(() => {
              useNotify({ title: '主页设置失败！', type: 'error' });
            });
        },
        cancel: () => {
          state.checked = false;
