<template>
  <plugin-setting v-if="isShow" :title="state.title" class="page-plugin-setting">
    <template #header>
      <button-group>
        <svg-button name="save|svg" @click="savePageSetting" tips="保存页面配置"></svg-button>
        <svg-button v-if="!pageSettingState.isNew" name="delete|svg" tips="删除页面" @click="deletePage"></svg-button>
        <svg-button
          v-if="!pageSettingState.isNew"
          name="text-copy-page|svg"
          placement="bottom"
          tips="复制页面"
          @click="copyPage"
        ></svg-button>
      </button-group>
      <svg-button class="close-plugin-setting-icon" name="close|svg" @click="cancelPageSetting"></svg-button>
    </template>

    <template #content>
      <div class="page-setting-content">
        <tiny-collapse v-model:activeKey="state.activeName" :bordered="false">
          <tiny-collapse-item header="基本设置" :key="PAGE_SETTING_SESSION.general">
          ...
          <tiny-collapse-item
            ...
            header="输入输出"
            :key="PAGE_SETTING_SESSION.inputOutput"
          >
            <page-input-output></page-input-output>
            <template #extra>
              <Popover placement="topRight" trigger="hover">
                <template #content>
                  <Alert
                    type="info"
                    message="输入配置: 传递给页面的参数(类似组件的输入属性);输出配置: 页面传递出的事件(类似组件触发的输出事件)"
                    :closable="false"
          ...
          <tiny-collapse-item
            ...
            v-if="pageSettingState.currentPageData.group !== 'public'"
            header="页面生命周期配置"
            :key="PAGE_SETTING_SESSION.lifeCycles"
          >
            <life-cycles
            ...
            <template #extra>
              <Popover placement="topRight" trigger="hover">
                <template #content>
                  <Alert type="info" :message="lifeCycleTips" :closable="false" class="life-cycle-alert"></Alert>
                </template>
                <Icon icon="ant-design:question-circle-outlined"></Icon>
              </Popover>
            </template>
          ...
          <tiny-collapse-item class="history-source" header="历史备份" :key="PAGE_SETTING_SESSION.history">
...
import { Button, Collapse, CollapsePanel, Input, Popover, Alert } from 'ant-design-vue';
import { Icon } from '@begcode/components';
import { PluginSetting, ButtonGroup, SvgButton, LifeCycles } from '@/common';
import { useLayout, usePage, useCanvas, useModal, useApp, useNotify, getGlobalConfig } from '@/controller';
import { extend, isEqual } from '@opentiny/vue-renderless/common/object';
import { constants } from '@/utils';
import { isVsCodeEnv } from '@/common/js/environments';
import { handlePageUpdate } from '@/common/js/http';
import { generatePage } from '@/common/js/vscodeGenerateFile';
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const { DEFAULT_PAGE, pageSettingState, changeTreeData, isCurrentDataSame, initCurrentPageData, isTemporaryPage } = usePage();
        ...
        exec: () => {
          ...
          requestDeletePage(id)
            .then(() => {
              ...
              closePageSettingPanel();
              useNotify({ title: '删除页面成功！', type: 'success' });
            })
            .catch(() => {
              useNotify({ title: '删除页面失败！', type: 'error' });
            });
        },
      });
    };
