  ...
  <plugin-panel title="国际化资源" :isCloseLeft="false" class="plugin-panel-i18n">
    <template #content>
      <div style="display: flex">
        ...
        <tiny-file-upload ref="upload" size="small" :auto-upload="false" :show-file-list="false" action="/" @change="handleChange">
        ...
        <div class="download-btn" @click="downloadFile">
          <Icon icon="generate-code|svg"></Icon>
          <tiny-button type="text"> 下载导入模板 </tiny-button>
        </div>
        <p v-show="isLoading && notEmpty"><span id="boxeight" class="i18n-loading"></span><span>正在导入，请稍后...</span></p>
      </div>
      <div class="language-plugin-table">
        <div class="language-search-box">
          <tiny-select v-model="currentSearchType" :options="i18nSearchTypes" style="width: 150px"></tiny-select>
          <tiny-input v-model="searchKey" class="plugin-i18n-search" placeholder="请输入关键字" type="text" clearable>
            <template #prefix>
              <span class="icon">
                <Icon icon="basic-search|svg"></Icon>
              </span>
            </template>
          </tiny-input>
        </div>
        ...
        >
          ...
          <tiny-grid-column width="90" field="operation" title="操作">
            <template v-slot="data">
              <div v-if="editingRow !== data.record" class="i18n-opera">
                <tiny-tooltip class="item" effect="dark" placement="bottom" content="编辑" :open-delay="500">
                  <span class="icon">
                    <svg-icon name="edit" @click.stop="openEditor($event, data.record)"></svg-icon>
                  </span>
                </tiny-tooltip>
                <tiny-tooltip class="item" effect="dark" placement="bottom" :open-delay="500">
                  ...
                  <tiny-popover
                    ...
                    :modelValue="data.record.key && data.rowIndex === copyRowIndex"
                  >
                    <template #content>
                      <span class="icon">
                        <svg-icon name="copy" @click="copyId(data.record, data.rowIndex)"></svg-icon>
                      </span>
                    </template>
                  </tiny-popover>
                </tiny-tooltip>
                <tiny-tooltip class="item" effect="dark" placement="bottom" title="删除" :open-delay="500">
                  <span class="icon">
                    <svg-icon name="delete" @click="openDeletePopover(data.record)"></svg-icon>
                  </span>
                </tiny-tooltip>
              </div>
            </template>
          ...
          <template #empty>
            ...
            <div v-else class="empty-wrap">
              <Icon class="empty-icon" icon="empty|svg"></Icon>
...
import { Table, TableColumn, Input, Popover, Button, Upload, Spin, Tooltip, Select } from 'ant-design-vue';
import { iconLoadingShadow } from '@opentiny/vue-icon';
import { PluginPanel } from '@/common';
import { useTranslate, useApp, useModal, getGlobalConfig } from '@/controller';
import { utils } from '@/utils';
import { useHttp } from '@/http';
import { BASE_URL } from '@/common/js/environments';
  ...
  setup() {
    ...
    const sortByLetter = (sortType = 'zh_CN') => {
      langList.value = langList.value.sort((a, b) => {
        ...
        else return sortType === 'zh_CN' ? a[sortType].localeCompare(b[sortType], 'zh') : a[sortType].localeCompare(b[sortType]);
      });
    };
  ...
  :deep(.tiny-grid) {
    ...
    .tiny-grid__body-wrapper {
      ...
      .tiny-grid-body__row:not(.row__hover):nth-child(2n) {
        background-image: linear-gradient(-180deg, var(--ti-lowcode-tabs-border-color), var(--ti-lowcode-tabs-border-color));
