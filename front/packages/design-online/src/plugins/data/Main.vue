<template>
  <div id="data-source">
    <div class="data-source-left-panel">
      ...
      <tiny-tabs v-model:activeKey="activeName" @click="tabClick" tab-style="button-card">
        <tiny-tab-item v-if="isBlock" :key="STATE.CURRENT_STATE" tab="区块状态"></tiny-tab-item>
        <tiny-tab-item v-else :key="STATE.CURRENT_STATE" tab="页面状态"></tiny-tab-item>
        <tiny-tab-item :key="STATE.GLOBAL_STATE" tab="应用状态"></tiny-tab-item>
      </tiny-tabs>
      <tiny-search :modelValue="query" class="left-filter" placeholder="请输入搜索条件" @update:modelValue="search"></tiny-search>
      <div class="add-btn">
...
import { Button, InputSearch, Tabs, TabPane } from 'ant-design-vue';
import { useCanvas, useHistory, useEditorInfo, useResource, useNotify, useData, useLayout } from '@/controller';
import { setState, getSchema, deleteState, setGlobalState, getGlobalState } from '@/canvas';
...
import { RadioButtonGroup } from '@begcode/components';
