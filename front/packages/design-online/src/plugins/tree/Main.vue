  ...
  <plugin-panel class="outlinebox" title="大纲树" @close="$emit('close')">
    ...
    <template #content>
      <div class="tree-wrap lowcode-scrollbar">
        <tiny-grid
          ref="gridRef"
          :data-source="state.pageSchema"
          :tree-config="{ children: 'children', expandAll: true, renderIcon: renderIconFn }"
          :show-header="false"
        ...
        >
          <tiny-grid-column data-index="componentName" tree-node>
            <template #default="data">
              <span
                class="tree-box"
                :schemaId="data.record?.id"
                :type="data.record.componentName"
                @mouseover="mouseover(data.record, $event)"
                @mouseleave="mouseleave(data.record, $event)"
                @click="checkElement(data.record)"
              >
                <span class="tree-content">
                  <!-- <span class="node-icon">
                    <component :is="getIcon(data.row)" style="width: 1em; height: 1em"></component>
                  </span> -->
                  <span>{{ data.record.componentName }}</span>
                </span>
                <span v-show="!data.record.show" class="tree-handle" @mouseup="showNode(data.record)">
                  <icon-eyeopen v-show="!data.record.show"></icon-eyeopen>
                  <icon-eyeclose v-show="data.record.show"></icon-eyeclose>
                </span>
              </span>
            </template>
          </tiny-grid-column>
...
import { useCanvas, useResource, useLayout } from '@/controller';
import { extend } from '@opentiny/vue-renderless/common/object';
import { typeOf } from '@opentiny/vue-renderless/common/type';
import { getRenderer, clearSelect, selectNode, hoverNode } from '@/canvas';
...
export default {
  ...
  setup() {
    ...
    const state = reactive({
      ...
      expandAll: true,
      initSchema: [],
      isLock: computed(() => ![PAGE_STATUS.Occupy, PAGE_STATUS.Guest].includes(useLayout().layoutState.pageStatus.state)),
      dragState: {
        allowDrop: true,
        dragSchema: null,
        hoverVm: null,
    ...
    const getClassName = record => {
      const hightLight = pageState?.currentSchema?.id === record.id ? 'high-light-node' : '';
    ...
    const rowDropMove = (event, originalEvent) => {
      ...
      if (
        ...
        el.querySelector('.tiny-grid-tree-wrapper').innerHTML = '<i class="tiny-grid-tree__node-btn tiny-grid-icon__caret-right"></i>';
      }

      if (state.dragState.parentNode && state.dragState.parentNode.getAttribute('data-rowid') !== el.getAttribute('data-rowid')) {
