  ...
  if (JDL.applications.length > 0) {
    ...
    nomlEntities = JDL.applications.map(app => {
      ...
      const [children, unProcessed] = processEntities(
        JDL,
        unProcessedEntities,
        findActualEntities(app.entities.entityList, app.entities.excluded, JDL.options.microservice, app.config.baseName, allEntityNames),
      );
      ...
      return {
        ...
        renderType: '<package>',
        description: `${app.config.clientFramework ? `Client: ${app.config.clientFramework}, ` : ''}Auth: ${
          app.config.authenticationType
        }, DB: ${app.config.prodDatabaseType}`,
        children,
      };
    });
  }
  if (unProcessedEntities.length > 0) {
    const [processed] = processEntities(JDL, unProcessedEntities, unProcessedEntities);
...
function processEntities(JDL, unProcessed, filter: string[] = []): [NomlEntity[], string[]] {
  ...
  JDL.entities.forEach(entity => {
    ...
    out.push({
      ...
      children: entity.body.map(field => {
        ...
        return {
          name: `${field.name} : ${field.type}${field.validations.filter(it => it.key === 'required').length === 1 ? '*' : ''}`,
  ...
  JDL.relationships.forEach(rel => {
    if (filter.length > 0 && (![...filter, 'User'].includes(rel.from.name) || ![...filter, 'User'].includes(rel.to.name))) {
      return;
    }
    out.push({
      name: `[${rel.from.name}] ${getCardinality(rel.cardinality)} [${rel.to.name}]`,
      type: NomlEntityTypes.relation,
      renderType: '',
    });
  });
...
function mapToNoml(it: NomlEntity) {
  switch (it.type) {
    ...
    case NomlEntityTypes.relation:
      return `${it.name}\n`;
    default:
      return `[${it.renderType}${it.name}|${it.description ? `${it.description}|` : ''}\n${it.children?.map(mapToNoml).join('\n')}\n]`;
  }
}
