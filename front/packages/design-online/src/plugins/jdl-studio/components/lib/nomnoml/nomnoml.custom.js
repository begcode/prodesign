...
nomnoml.Classifier = function (type, name, compartments) {
  if (type === 'ENUM') {
    var enumLabel = cloneDeep(compartments[0]),
      label = '<<enumeration>>';
...
nomnoml.layout = function (measurer, config, ast) {
  function runDagre(input) {
    return dagre.layout().rankSep(config.spacing).nodeSep(config.spacing).edgeSep(config.spacing).rankDir(config.direction).run(input);
  }
  function measureLines(lines, fontWeight) {
    ...
    return {
      width: Math.round(max(map(lines, measurer.textWidth)) + 2 * config.padding),
  ...
  function layoutCompartment(c, compartmentIndex) {
    ...
    each(c.nodes, layoutClassifier);

    var g = new dagre.Digraph();
    each(c.nodes, function (e) {
      g.addNode(e.name, { width: e.width, height: e.height });
    });
    each(c.relations, function (r) {
      g.addEdge(r.id, r.start, r.end);
    });
    var dLayout = runDagre(g);

    var rels = keyBy(c.relations, 'id');
    var nodes = keyBy(c.nodes, 'name');
    function toPoint(o) {
      return { x: o.x, y: o.y };
    }
    ...
    dLayout.eachEdge(function (e, u, v, value) {
      ...
      rels[e].path = map(flatten([start, value.points, end]), toPoint);
    });
  ...
  function layoutClassifier(clas) {
    each(clas.compartments, layoutCompartment);
    clas.width = max(map(clas.compartments, 'width'));
    ...
    each(clas.compartments, function (co) {
      co.width = clas.width;
    });
  }
...
nomnoml.parse = function (source) {
  ...
  var pureDirectives = filter(lines, isDirective);
  var directives = zipObject(
  ...
  var pureDiagramCode = map(map(lines, 'text'), onlyCompilables).join('\n').trim();
...
nomnoml.convertToNomnoml = function (JDLObj) {
  ...
  var isRequired = function (validations) {
    return filter(validations, required).length > 0;
  };
  var setEnumRelation = function (a, part) {
    var enumPart = filter(enumParts, function (e) {
      return e.id === a.type;
    });
  ...
  var setParts = function (entity, isEnum) {
    var attrs = [];
    if (isEnum) {
      each(entity.values, function (a) {
        attrs.push(a);
      });
    } else {
      each(entity.body, function (a) {
  ...
  each(JDLObj.entities, function (p) {
    if (p.name) {
      // is a classifier
      var part = setParts(p);
      parts.push(part);
      each(p.body, function (a) {
        setEnumRelation(a, part);
      });
    }
  });

  each(JDLObj.relationships, function (p) {
    parts.push({
      assoc: '->',
...
nomnoml.transformParseIntoSyntaxTree = function (entity) {
  ...
  function transformCompartment(parts) {
    ...
    var allClassifiers = map(rawClassifiers, transformItem);
    var noDuplicates = map(groupBy(allClassifiers, 'name'), function (cList) {
      return max(cList, function (c) {
  ...
  function transformItem(entity) {
    ...
    if (isArray(entity)) return transformCompartment(entity);
    if (entity.parts) {
      var compartments = map(entity.parts, transformCompartment);
...
nomnoml.render = function (graphics, config, compartment, setFont) {
  ...
  function renderCompartment(compartment, style, level) {
    ...
    each(compartment.lines, function (text, i) {
    ...
    each(compartment.relations, function (r) {
      renderRelation(r, compartment);
    });
    each(compartment.nodes, function (n) {
      renderNode(n, level);
    });
    g.ctx.restore();
  }
  ...
  function renderNode(node, level) {
    ...
    var shade = config.fill[level] || last(config.fill);
    ...
    each(node.compartments, function (part, i) {
  ...
  function strokePath(p) {
    if (config.edges === 'rounded') {
      ...
      g.ctx.lineTo(last(p).x, last(p).y);
      g.ctx.stroke();
  ...
  function renderRelation(r, compartment) {
    var startNode = find(compartment.nodes, { name: r.start });
    var endNode = find(compartment.nodes, { name: r.end });

    var start = rectIntersection(r.path[1], first(r.path), startNode);
    var end = rectIntersection(r.path[r.path.length - 2], last(r.path), endNode);

    var path = flatten([start, tail(initial(r.path)), end]);
    ...
    drawArrowEnd(first(tokens), path.reverse(), start);
  }
  ...
  function rectIntersection(p1, p2, rect) {
    ...
    for (var t = 1; t >= 0; t -= 0.01) {
      var p = vm.mult(v, t);
      if (Math.abs(p.x) <= rect.width / 2 + config.edgeMargin && Math.abs(p.y) <= rect.height / 2 + config.edgeMargin) return vm.add(p2, p);
    }
    return p2;
  }
  ...
  function drawArrow(path, isOpen, arrowPoint, diamond) {
    ...
    var v = vm.diff(path[path.length - 2], last(path));
