import { identity, flatten, zip } from 'lodash-es';
var skanaar = skanaar || {};
...
skanaar.Canvas = function (canvas, callbacks) {
  ...
  function tracePath(path, offset, s) {
    ...
    for (var i = 1, len = path.length; i < len; i++) ctx.lineTo(offset.x + s * path[i].x, offset.y + s * path[i].y);
    return chainable;
  }
  ...
  return {
    ...
    radialGradient: function (x, y, r1, r2, colors) {
      ...
      for (var key in colors) if (colors.hasOwnProperty(key)) grad.addColorStop(key, colors[key]);
      return grad;
    },
  };
};
...
skanaar.format = function format(template /* variadic params */) {
  ...
  return flatten(zip(template.split('#'), parts)).join('');
};
