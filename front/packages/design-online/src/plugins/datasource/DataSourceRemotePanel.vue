      ...
      <template #content>
        <div class="create-config">
          <data-source-remote-form
            ...
            ref="dataSourceRemoteForm"
          ></data-source-remote-form>
          <data-source-remote-autoload v-model="state.remoteData.options.isSync"></data-source-remote-autoload>
          <div class="tabBox">
            <Tabs v-model:activeKey="state.activeNameTabs">
              <TabPane class="json-tab" tab="请求JSON参数" key="jsonsTab" :forceRender="true">
                <data-source-remote-parameter v-model="state.remoteData.options.params"></data-source-remote-parameter>
              </TabPane>
              <TabPane key="responseTab" :forceRender="true">
                <template #tab>
                  <span
                    >请求处理
                    <Popover>
                      <template #content>
                        <p>* 如果接口存在跨域、鉴权等情况，则请手动将响应数据，粘贴至下方的“请求结果”编辑器中。</p>
                        <p>
                          *
                ...
                <data-source-remote-adapter
                  :ref="el => (dataSourceRemoteAdapterRef = el)"
          ...
          <div>
            <Collapse v-model:activeKey="state.activeName">
              <CollapsePanel key="result" header="请求结果">
                <data-source-remote-data-result v-model="state.remoteData.result"></data-source-remote-data-result>
              </CollapsePanel>
            </Collapse>
          </div>
        </div>
...
import { Collapse, CollapsePanel, Tabs, TabPane, Popover } from 'ant-design-vue';
import { PluginSetting } from '@/common';
import DataSourceRemoteForm from './DataSourceRemoteForm.vue';
...
import DataSourceRemoteDataResult, { getResponseData } from './DataSourceRemoteDataResult.vue';
import { open as openRemoteMapping } from './DataSourceRemoteMapping.vue';
import { useModal, useDataSource } from '@/controller';
import { isEmptyObject } from '@opentiny/vue-renderless/common/type';
import { obj2String, string2Obj } from '@/controller/adapter';
...
export default {
  ...
  emits: ['confirm'],
  setup(props, { emit }) {
    const dataSourceRemoteAdapterRef = ref(null);
    const dataSourceRemoteForm = ref(null);
    ...
    const saveRemote = () => {
      // 远程表单校验
      dataSourceRemoteForm.value.getServiceForm().validate(valid => {
        if (valid) {
          ...
          const save = () => {
            ...
            dataSourceState.remoteConfig = {
              ...
              ...dataSourceRemoteAdapterRef.value.getEditorValue(),
            };
    ...
    const sendRequest = () => {
      ...
      const request = getRequest({ options, ...dataSourceRemoteAdapterRef.value.getEditorValue() });
