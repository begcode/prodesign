...
import { isMock } from '@/common/js/environments';
import { utils as commonUtils, constants } from '@/utils';
...
export const getRequest = config => {
  const globalDataHandle = useResource().resState.dataHandler ? createFn(useResource().resState.dataHandler.value) : defaultDataHandler;

  const globalErrorHandler = useResource().resState.errorHandler
    ? createFn(useResource().resState.errorHandler.value)
    : defaultErrorHandler;

  const globalWillFetch = useResource().resState.willFetch ? createFn(useResource().resState.willFetch.value) : defaultWillFetch;
