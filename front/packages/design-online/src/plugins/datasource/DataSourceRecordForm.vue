<template>
  <plugin-setting v-if="isOpen" :title="title" :is-second="true" @cancel="closeRecordFormPanel" @save="saveRecordListData">
    <template #content>
        ...
        <slot>
          ...
          >
            <tiny-form-item v-for="item in state.filterData" :key="item.id" :prop="item.name">
              ...
              <component :is="state.components[item.type]" v-model="recordFormData[item.name]" :type="getRenderType(item.format)" />
            </tiny-form-item>
          </tiny-form>
        </slot>
...
import { Button, Input, InputNumber, DatePicker, Form, FormItem, Switch, Slider } from 'ant-design-vue';
  ...
  setup(props, { emit }) {
    ...
    watch(
      ...
      value => {
        ...
        dataSourceState.isRecordValidate = Object.keys(state.recordMapping).every(key => state.recordMapping[key] && value[key]);
      },
      { deep: true },
    );
