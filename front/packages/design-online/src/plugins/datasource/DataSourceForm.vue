    ...
    <template #header>
      <button-group>
        ...
        <div class="field-handler" @click="deleteDataSource" v-if="editable" tips="删除">
          <svg-button name="text-source-delete|svg"></svg-button>
        </div>
      </button-group>
      <svg-button class="close-plugin-setting-icon" name="close|svg" @click="closeAllPanel"></svg-button>
    </template>
    <template #content>
...
import { ButtonGroup, PluginSetting, SvgButton } from '@/common';
...
import { useModal, useApp, useDataSource } from '@/controller';
