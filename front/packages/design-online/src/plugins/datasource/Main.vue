<template>
  <plugin-panel title="数据源">
    <template #header>
      <svg-button
        class="add-page"
        tips="新建数据源"
        name="text-source-list-add|svg"
        @click="openDataSourceFormPanel()"
        placement="top"
      ></svg-button>
      <svg-button class="set-page" tips="全局设置" name="text-source-setting|svg" @click="openGlobalDataHanderPanel"></svg-button>
      <svg-button class="refresh-page" tips="刷新数据源" name="text-source-list-ref|svg" @click="refreshDataSource"></svg-button>
    </template>
    <template #content>
  ...
  ></data-source-remote-panel>
  <data-source-form v-model="state.currentDataSource" :editable="state.editable" @save="refreshDataSource"></data-source-form>
...
import DataSourceRemotePanel, { close as closeRemotePanel, isOpen as isOpenRemotePanel } from './DataSourceRemotePanel.vue';
...
import { useDataSource } from '@/controller';
...
export default {
  ...
  emits: ['fixPanel', 'close', 'fixed-panels'],
  setup() {
