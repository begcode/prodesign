<template>
  <div class="right-item">
    <div class="title">
      <span class="title">数据源字段<tiny-button class="newJsonBtn" @click="openNewFieldForm">新建字段</tiny-button> </span>
      <div class="btn">
        <tiny-button type="text" @click="$emit('openRemotePanel')">{{ editable ? '修改远程配置' : '获取远程字段' }}</tiny-button>
      </div>
    </div>
    <div class="section">
...
export default {
  ...
  emits: ['update:modelValue', 'openPanel', 'openRemotePanel'],
  setup(props, { emit }) {
    const format = {
      required: false,
      stringType: '',
