<template>
  <div v-if="state.isOpen" class="step-select-second">
    <div class="field-row">
      ...
      <span v-if="editable">
        <button-group>
          <Button type="text" @click.stop="saveField">保存</Button>
    ...
    <div v-if="editable" class="field-content">
      <Form ref="form" label-position="left" :rules="rules" :model="state.field" validate-type="text">
        <FormItem class="title-content" name="title" label="字段名称" label-width="150px">
          <meta-bind-i18n v-model="state.field.title"></meta-bind-i18n>
        </FormItem>
        <FormItem class="name-content" name="name" label="字段唯一标识" label-width="150px">
          <Input class="filedName" v-model="state.field.name"></Input>
        </FormItem>
        <!--不同的字段类型对应不同的校验规则-->
...
import { ButtonGroup, MetaBindI18n, SvgButton } from '@/common';
