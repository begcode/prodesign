<template>
  <plugin-panel title="资源管理" :isCloseLeft="false" @close="closePanel">
    <template #header>
      <svg-button name="add-utils|svg" placement="left" :tips="tips" @click="addResource('npm')"></svg-button>
    </template>
    <template #content>
      <Tabs
        v-model:activeKey="activedName"
        class="tabs full-width-tabs"
        tab-style="button-card"
        @click="switchTab"
        style="margin-left: 8px"
      >
        <TabPane :key="RESOURCE_TYPE.Util" tab="工具类">
          <bridge-manage ref="utils" :name="RESOURCE_TYPE.Util" @open="openBridgePanel"></bridge-manage>
        </TabPane>
        <TabPane v-if="isVsCodeEnv" :key="RESOURCE_TYPE.Bridge" tab="桥接源">
