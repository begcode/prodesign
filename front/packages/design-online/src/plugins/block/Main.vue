<template>
  <plugin-panel class="block-manage" title="区块管理" :isCloseLeft="false" @close="closePanel">
    <template #header>
      <svg-button name="add-page|svg" placement="bottom" tips="新建区块" style="cursor: not-allowed"></svg-button>
    </template>
    <template #content>
      <div class="app-manage-search">
        <tiny-select
          ...
          class="search-select"
        >
          <tiny-option v-for="item in categoryList" :key="item.id" :title="item.name" :value="item.id" class="block-group-option-item">
            <div class="block-item">
              <span>{{ item.name }}</span>
              <div class="item-btns">
                <svg-button class="item-icon" name="edit|svg" title="编辑" @click.stop="editCategory(item)"></svg-button>
                <tiny-popover
                  ...
                  <template #content>
                    <div class="popper-confirm" @mousedown.stop="">
                      <div class="popper-confirm-header">
                        <Icon class="icon" icon="warning|svg"></Icon>
                        <span class="title">您确定删除该区块分类吗？</span>
                      </div>
                      <div class="popper-confirm-footer">
                        <tiny-button class="confirm-btn" size="small" type="primary" @click="delCategory(item.id)">确定</tiny-button>
                        <tiny-button class="cancel-btn" size="small" @click="handleShowDeleteModal(null)">取消</tiny-button>
                      </div>
                    </div>
                  </template>
                </tiny-popover>
        ...
        <svg-button class="add-group-btn" tips="新建分类" name="add-page|svg" @click="createCategory"></svg-button>
      </div>
      <div class="app-manage-search">
        ...
        <tiny-dropdown trigger="click" @item-click="changeType">
          ...
          <template #overlay>
            <tiny-dropdown-menu placement="top">
              <SubMenu :key="item.key" :title="item.label" v-for="item in state.sortOptions" />
            </tiny-dropdown-menu>
          </template>
        </tiny-dropdown>
...
import {
  InputSearch as TinySearch,
  Select as TinySelect,
  SelectOption as TinyOption,
  Dropdown as TinyDropdown,
  // DropdownMenu as TinyDropdownMenu,
  Menu as TinyDropdownMenu,
  Popover as TinyPopover,
  Button as TinyButton,
  SubMenu,
} from 'ant-design-vue';
import { PluginPanel, PluginBlockList, SvgButton, SaveNewBlock } from '@/common';
import { useBlock, useModal, useLayout, useCanvas } from '@/controller';
...
export default {
  ...
  setup() {
    ...
    const state = reactive({
      ...
      publishFilterType: '',
      sortOptions: [
        { label: '按时间正序', key: SORT_TYPE.timeAsc },
        { label: '按时间倒序', key: SORT_TYPE.timeDesc },
        { label: '按字母正序', key: SORT_TYPE.alphabetAsc },
        { label: '按字母倒序', key: SORT_TYPE.alphabetDesc },
