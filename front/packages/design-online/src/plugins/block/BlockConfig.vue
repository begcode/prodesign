<template>
  <tiny-form ref="blockForm" class="block-setting-content" :model="formData" :rules="rules">
    <tiny-form-item label="区块名称" prop="name_cn">
      <div>
        <tiny-input v-model="formData.name_cn" placeholder="请输入区块名称" @blur="changeBlockProperty('name_cn')"></tiny-input>
      </div>
    </tiny-form-item>
    ...
    <tiny-form-item label="区块描述" prop="description">
      <div>
        <tiny-input v-model="formData.description" placeholder="请输入区块描述" @blur="changeBlockProperty('description')"></tiny-input>
      </div>
    </tiny-form-item>
    ...
    <tiny-form-item label="区块标签" prop="tags">
      <div class="block-tag-create">
        <tiny-tag v-for="(tag, index) in formData.tags" :key="index" closable class="tag-button" @close="deleteTag(tag)">
        ...
        <tiny-button v-show="!state.inputVisible" class="button-new-tag" size="small" @click="addTag"> + 标签 </tiny-button>
      </div>
    ...
    <tiny-form-item label="公开范围" prop="openness">
      <div class="block-openness">
        <div class="block-openness-public">
          <tiny-radio v-model="formData.public" :label="BLOCK_OPENNESS.Private" @change="changeOpenness">私有</tiny-radio>
          <tiny-radio v-model="formData.public" :label="BLOCK_OPENNESS.Open" @change="changeOpenness">公开</tiny-radio>
          <tiny-radio v-model="formData.public" :label="BLOCK_OPENNESS.Special" @change="changeOpenness">半公开</tiny-radio>
          <div v-show="formData.public === BLOCK_OPENNESS.Special" class="block-openness-tenants">
            ...
            >
              <tiny-option v-for="item in state.publicOptions" :key="item.id" :label="item.name_cn || item.tenant_id" :value="item.id">
              </tiny-option>
            </tiny-select>
          </div>
        </div>
...
import { Input, Tag, Button, Form, FormItem, Radio, Select, SelectOption } from 'ant-design-vue';
import { constants } from '@/utils';
import { remove } from '@opentiny/vue-renderless/common/array';
import { getEditBlock } from './js/blockSetting';
import { useBlock, useEditorInfo } from '@/controller';
