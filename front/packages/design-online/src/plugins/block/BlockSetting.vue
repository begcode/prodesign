<template>
  <plugin-setting v-if="isOpen" class="plugin-block-setting" title="设置" @mouseleave="onMouseLeave" @click="handleClick">
    <template #header>
      ...
      <tiny-button type="primary" :disabled="globalConfig.dslMode === 'Angular'" class="publish-btn" @click="showDeployBlockDialog"
        >发布
      </tiny-button>
    ...
    <template #content>
      <tiny-collapse v-model="state.activeName">
        ...
        <tiny-collapse-item title="版本列表" name="history">
          <block-history-list :is-block-manage="true" :history="state.backupList" @preview="previewHistory"></block-history-list>
        </tiny-collapse-item>
      </tiny-collapse>
    </template>
  </plugin-setting>
  <block-deploy-dialog v-model:open="state.showDeployBlock" :nextVersion="nextVersion"></block-deploy-dialog>
</template>

<script lang="jsx">
import { reactive, ref, watch, watchEffect, computed } from 'vue';
import { Button as TinyButton, Collapse as TinyCollapse, CollapsePanel as TinyCollapseItem } from 'ant-design-vue';
import { getGlobalConfig, useModal } from '@/controller';
import { BlockHistoryList, PluginSetting, CloseIcon, SvgButton } from '@/common';
import { previewBlock } from '@/common/js/preview';
import { LifeCycles } from '@/common';
import BlockEvent from './BlockEvent.vue';
import BlockConfig from './BlockConfig.vue';
import BlockProperty from './BlockProperty.vue';
import { getEditBlock, delBlock, saveBlock, DEPLOY_TIPS, getBlockBase64, setConfigItemVisible, saveArrayConfig } from './js/blockSetting';
import { BlockDeployDialog } from '@/common';
