...
export const handleSaveWidgetProps = data => {
  ...
  try {
    ...
    useNotify({
      type: 'error',
      title: `属性面板组件属性值设置有误: ${JSON.stringify(error)}，请检查是否为JSON类型`,
    });
  }
};
...
watch(
  ...
  () => {
    ...
    showArrayItemConfig.value = type.value === META_TYPES.array && widgetComponent.value === META_COMPONENTS_ENUM.MetaArrayItem;
