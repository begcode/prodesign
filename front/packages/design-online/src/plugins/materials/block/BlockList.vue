...
import { useBlock, useResource, useModal, useApp } from '@/controller';
import { PluginBlockList } from '@/common';
import { dragStart, addComponent } from '@/canvas';
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const { isDefaultGroupId, isAllGroupId, selectedBlock, selectedGroup, isRefresh, getBlockAssetsByVersion } = useBlock();
    ...
    const openDetail = ({ item, index }) => {
      ...
      const offsetWidth = panelState.isBlockGroupPanel && panelState.isBlockList ? -(itemOffsetWidth / 8) : itemOffsetWidth / 4;
      const location = index % 2 ? `calc(50% + ${itemOffsetWidth / 2 + 'px'})` : `calc(50% - ${itemOffsetWidth / 2 + 'px'})`;
      ...
      const exec = () => {
        fetchGroupBlocksById({ groupId })
          .then(data => {
            ...
            const blocks = data?.filter(item => item.id !== blockId).map(block => ({ id: block.id, version: block.current_version }));
