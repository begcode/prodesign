<template>
  <div :class="['block-detail-panel', { 'no-backup': !detail?.backup }]">
    ...
    <div class="block-detail-content">
      ...
      <div v-if="!isSettingPanel && !isShortcutPanel" class="block-detail-item block-detail-show-more" @click="openHistory">
        <span>查看历史备份</span>
    ...
    const deleteBlock = blockId => {
      ...
      const exec = () => {
        fetchGroupBlocksById({ groupId })
          .then((data = []) => {
            const blocks = data.filter(item => item.id !== blockId).map(item => ({ id: item.id, version: item.current_version }));
