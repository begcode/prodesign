<template>
  <plugin-setting v-if="panel.created" v-show="panel.show" :title="selectedGroup.groupName" @cancel="closeGroupPanel" @save="addBlocks">
    <template #content>
...
import { useApp, useBlock, useModal, useResource } from '@/controller';
import BlockGroupTransfer from './BlockGroupTransfer.vue';
import BlockGroupFilters from './BlockGroupFilters.vue';

import { requestUpdateGroup, fetchAvailableBlocks, fetchGroupBlocksById, fetchTags, fetchTenants, fetchUsers } from './http';
...
export default {
  ...
  setup() {
    ...
    const selectedBlockFilter = blocks => {
      ...
      const isSelectedBlock = block => selectedBlockArray?.value?.some(selectedBlock => block.id === selectedBlock.id);
      ...
      Promise.allSettled([fetchTenants(), fetchUsers(), fetchTags()]).then(results => {
        ...
        state.filters[1].children =
          results[1].status === 'fulfilled'
            ...
            : [];
        state.filters[2].children = results[2].status === 'fulfilled' ? results[2].value.map(item => ({ name: item })) : [];
      });
    };
