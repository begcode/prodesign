...
import { getGlobalConfig, useApp } from '@/controller';
...
// 区块消费侧 -- 获取区块分组列表
export const fetchGroups = appId => http.get(`http://localhost:3290/material-center/api/block-groups?app=${appId}`);
...
export const fetchGroupBlocksByIds = async ({ groupIds }) => {
  const blockGroups = await http.get(
    `http://localhost:3290/material-center/api/block-groups${getParams({
      id: groupIds,
    })}`,
  );
...
// 创建区块分组信息
export const requestCreateGroup = ({ name, app }) => http.post('/material-center/api/block-groups/create', { name, app });
