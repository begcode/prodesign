<template>
  <div class="popover-head">
    <span>生命周期</span>
    <Popover v-model:open="state.showPopover" placement="leftBottom" trigger="hover">
      <template #content>
  ...
  <Modal
    ...
    width="48%"
    :append-to-body="true"
    @update:open="state.showDialog = $event"
    @close="cancel"
    @opened="open"
  >
    <div class="dialog-content">
      ...
      <div class="dialog-content-right">
        <monaco-editor v-if="state.showEditor" ref="editor" style="height: 100%" :options="state.options" :value="state.editorValue" />
      </div>
    </div>
...
import { Button, Modal, Popover, InputSearch, Tooltip } from 'ant-design-vue';
import { VueMonaco } from '@/common';
import { useCanvas, getGlobalConfig } from '@/controller';
...
export default {
  ...
  setup() {
    ...
    const openDialog = item => {
      state.title = item;
      state.editorValue =
        (state.bindLifeCycles[state.title] && state.bindLifeCycles[state.title].source) || `function ${state.title} () {} `;
