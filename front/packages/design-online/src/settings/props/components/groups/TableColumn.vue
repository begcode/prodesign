<template>
  <div class="colums">
    <div class="columsList">
      <meta-list>
        ...
        <template #items>
          <meta-list-items v-bind="itemsOptions" @deleteItem="deleteItem" @changeItem="dragEnd" @hide="hide" @openModal="openColumnSetting">
          </meta-list-items>
        </template>
      </meta-list>
    </div>
  ...
  <table-modal v-if="modal.created" v-show="modal.show" @close="closePanel">
    <table-setting-panel :properties="properties" :values="state.columnValues" :namePrefix="state.namePrefix"></table-setting-panel>
  </table-modal>
</template>
...
import { useModal, MetaModal, MetaList, MetaListItems, MetaListActions } from '@/common';
