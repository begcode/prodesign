  ...
  <div class="properties-wrap setting-panel">
    ...
    <template v-if="store.currentProperty?.label?.text">
      ...
      <div>
        ...
        <tiny-select v-model="store.currentProperty.type" :options="META_TYPES_OPTIONS" @change="handleTypeChange"></tiny-select>
      </div>
      <div>
      ...
      <div>
        ...
        <tiny-radio-group v-model="store.currentProperty.labelPosition" class="setting-radio-group setting-layout" size="mini">
      ...
      <div>
        ...
        <tiny-select v-model="store.currentProperty.widget.component" :options="options" @change="handleTypeChange"></tiny-select>
      </div>
...
import { Input, Select, Switch, RadioButton, RadioGroup, Tooltip } from 'ant-design-vue';
import { MetaListItems } from '@/common';
import { iconPlusCircle } from '@opentiny/vue-icon';
import store, { drawItems, META_TYPES_ENUM, META_TYPES_OPTIONS, META_COMPONENTS, DEFAULT_INIT_PROPERTIES } from '../store';
...
export default {
  ...
  setup() {
    const haveChild = computed(
      () => store.property?.items || store.property?.properties || store.property?.type === 'array' || store.property?.type === 'object',
    );
    ...
    const handleTypeChange = () => {
      const shouldShowArrayItem =
        store.currentProperty.type === META_TYPES_ENUM.array && store.currentProperty.widget.component === META_COMPONENTS.MetaArrayItem;
