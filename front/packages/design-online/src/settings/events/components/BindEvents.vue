  ...
  <div class="bind-action-list">
    <div class="popover-head">
      <span class="head-left">
        <Popover
          v-model:open="state.showEventAdditive"
          ...
          <template #content>
            <div class="custom-event" @click.stop>
              <Form
                ...
                label-position="left"
              >
                <FormItem label="事件函数名" prop="methodName" required>
                  <Input v-model="state.formData.methodName" placeholder="小驼峰格式，如：onDrag"></Input>
                </FormItem>
                <FormItem label="事件描述" prop="methodDescription" required>
                  <Input v-model="state.formData.methodDescription"></Input>
                </FormItem>
                <footer>
                  <Button @click="addMethod"> 确定</Button>
                  <Button @click="state.showEventAdditive = !state.showEventAdditive"> 取消</Button>
                </footer>
              </Form>
            </div>
          </template>
          <Button class="title add-custom-event-button" :reset-time="0" @click.stop="state.showEventAdditive = !state.showEventAdditive">
            <span class="custom-event-button-text">添加自定义事件</span>
            <Popover placement="bottomLeft" trigger="hover" popperClass="setting-advanced-add-custom-event-tip">
      ...
      <li v-for="action in state.bindActions" :key="action.eventName">
        <div class="action-item bind-action-item">
          ...
          <div class="action-buttons">
            ...
            <svg-button name="plugin-icon-page-schema|svg" tips="定位到代码" placement="top" @click="openCodePanel(action)"></svg-button>
            <svg-button name="setting|svg" tips="设置" placement="top" @click="openActionDialog(action, false)"></svg-button>
    ...
    <div v-show="isEmpty" class="empty-action">
      <div class="icon">
        <Icon icon="empty-action|svg" class="empty-action-icon"></Icon>
      </div>
...
import { Popover, Input, Form, FormItem, Button } from 'ant-design-vue';
import { useCanvas, useModal, useLayout, useBlock, useResource, useNotify } from '@/controller';
import { BlockLinkEvent, SvgButton } from '@/common';
  ...
  setup() {
    ...
    const addMethod = () => {
      if (state.componentEvents[state.formData.methodName]) {
        useNotify({
          type: 'error',
          title: `${state.formData.methodName}事件函数名已存在`,
        });
        return false;
      }
