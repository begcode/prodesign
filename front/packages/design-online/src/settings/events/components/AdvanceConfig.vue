    ...
    <div class="advnce-config">
      ...
      <div class="advanced-config-form-item">
        <tiny-tooltip content="建议填写循环项中的唯一值（如item.id），如果填写为数字将不保存">
          <meta-input v-model="state.loopKey" :placeholder="`默认为索引名：${getIndexName()}`" @update:modelValue="setLoopKey"></meta-input>
        </tiny-tooltip>
      </div>
    </div>
  </div>
...
import { MetaSwitch, MetaBindVariable, MetaInput, MetaCodeEditor } from '@/common';
import { useProperties, useCanvas } from '@/controller';
import { PROP_DATA_TYPE } from '@/controller/utils';
import { string2Obj } from '@/controller/adapter';
import { updateRect } from '@/canvas';
