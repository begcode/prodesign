<template>
  <div class="container-flex">
    <div v-for="(item, index) in layoutOpts" :key="index" class="flex-item">
      <span :class="['title', { 'is-setting': getSettingFlag(hyphenate(item.key)) }]" @click="openSetting(hyphenate(item.key), $event)">{{
        item.title
      }}</span>
      <div>
        <meta-radio :options="item.btnList" :value="item.picked" @pickedChange="args => select(args, item)"></meta-radio>
      </div>
    </div>
  </div>
