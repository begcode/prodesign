  ...
  <div class="grid-box-container">
      ...
      <template #content>
        <div class="grid-edit-wrap">
          <div class="grid-edit-spacing">
            <div class="gap">
              <span :class="['gap-label', { 'is-setting': getGapSetting() }]" @click="openSetting(GRID_PROPERTY.GridGap, $event)"
                >间隔</span
              >
              <div class="gap-input">
                <numeric-select
            ...
            <div class="direction">
              <span
                ...
                >方向</span
              >
              <meta-radio :options="state.direction" :value="state.picked" :disabled="true" @pickedChange="select"></meta-radio>
            </div>
            <div class="dense">
              <input id="dense" v-model="state.densePicked" type="checkbox" name="dense" value="dense" @change="denseChange" />
              <label for="dense">Dense</label>
              <Tooltip :open-delay="500" title="dense 表示尽可能紧密填满，尽量不出现空格">
            ...
            <div v-for="(item, index) in state.metaOptions" :key="index" class="layout-item">
              ...
              <meta-list-items :optionsList="item.list">
                ...
                <template #operate="{ data }">
                  <Tooltip class="item" effect="dark" :open-delay="500" title="复制" placement="top">
                    <span class="item-icon">
                      <icon-copy @click="copyItem(item.list, data)"></icon-copy>
                    </span>
                  </Tooltip>
                  <Tooltip class="item" effect="dark" :open-delay="500" title="删除" placement="top">
                    <span class="item-icon">
    ...
    <div class="grid-item-wrap">
      <div v-for="(item, index) in state.gridOptions" :key="index" class="grid-item">
        <span :class="['grid-label', { 'is-setting': getPropSetting(item.title) }]" @click="openSetting(item.title, $event)">{{
          item.title
        }}</span>
        <div class="radio-wrap">
          <meta-radio :options="item.align.list" :value="item.align.picked" @pickedChange="args => select(args, item.align)"></meta-radio>
          <meta-radio
...
import { MetaRadio, MetaListItems, MaskModal } from '@/common';
