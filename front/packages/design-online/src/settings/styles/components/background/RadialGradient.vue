<template>
  ...
  <div class="background-row line">
    ...
    <div class="row-content">
      ...
      >
        <div :class="['row-content-item', { selected: state.radialSize === item.value }]" @click="selectRadialSize(item)">
          <svg-icon :name="item.icon"></svg-icon>
        </div>
      </tiny-tooltip>
    </div>
  </div>
  <background-image-gradient :repeat="state.repeat" :colorList="state.colorList" @update="updateGradient"></background-image-gradient>
</template>

<script setup>
import { reactive, onMounted } from 'vue';
import { Tooltip as TinyTooltip } from 'ant-design-vue';
...
const updateStyle = () => {
  const property = {
    [BACKGROUND_PROPERTY.BackgroundImage]: `${state.repeat ? 'repeating-' : ''}radial-gradient(circle ${state.radialSize} at ${
