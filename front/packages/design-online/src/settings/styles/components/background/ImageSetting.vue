...
import { Tooltip as TinyTooltip, Checkbox as TinyCheckbox } from 'ant-design-vue';
import PositionOrigin from './PositionOrigin.vue';
import InputSelect from '../inputs/InputSelect.vue';
import { BACKGROUND_PROPERTY, UNIT_OPTIONS, BACKGROUND_SIZE_OPTIONS, FIXED_OPTIONS, REPEAT_OPTIONS } from '../../js/styleProperty';
...
const imageSizeChange = val => {
  ...
  val ? updateStyle({ [BACKGROUND_PROPERTY.BackgroundSize]: '125px' }) : updateStyle({ [BACKGROUND_PROPERTY.BackgroundSize]: null });
};
...
const setBackgroundSize = () => {
  ...
  const height = isAutoHeight ? state.height.toLocaleLowerCase() : `${state.height.toLocaleLowerCase() + state.heightSuffix}`;
