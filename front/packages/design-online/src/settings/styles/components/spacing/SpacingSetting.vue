  ...
  <div :class="['content-wrap', { 'is-margin': isMargin, 'is-padding': isPadding }]">
    ...
    <div class="content-value">
      ...
      <ul :class="['custom', { 'is-padding': isPadding }]">
        <li v-for="item in options" :key="item" :class="['option', { active: item === propertyValue }]" @click="select(item)">
          {{ item }}
        </li>
      </ul>
    </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const isMargin = computed(() => props.property.type === SPACING_PROPERTY.Margin || props.property.type === POSITION_PROPERTY.Position);
    const isPadding = computed(() => props.property.type === SPACING_PROPERTY.Padding);

    const propertyValue = computed(() => (props.property.value === 'auto' ? 'auto' : Number.parseInt(props.property.value)));
