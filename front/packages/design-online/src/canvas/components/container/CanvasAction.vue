<template>
  ...
  >
    <div v-if="!resize" ref="labelRef" class="corner-mark-left" :style="labelStyle">
      ...
      <TinyPopover v-model="showPopover" placement="top-start" title="快捷设置" width="310" popper-class="short-cut-set" trigger="manual">
    ...
    <div v-if="showAction" class="corner-mark-right" :style="fixStyle">
      ...
      <template v-else>
        ...
        <div title="显示">
          <icon-eyeopen @click.stop="show"></icon-eyeopen>
        </div>
      </template>
      <div title="删除">
...
import { IconDel, IconSetting, IconChevronLeft, IconArrowDown, IconArrowUp, IconCopy, IconEyeclose, IconEyeopen } from '@opentiny/vue-icon';
import {
  getCurrent,
  removeNodeById,
  selectNode,
...
export default {
  ...
  setup(props) {
    ...
    const show = () => {
      getRenderer().setCondition(getCurrent().schema?.id, true);
      updateRect();
    };
