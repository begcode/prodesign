...
const generateAssginColumns = (newColumns, oldColumns) => {
  ...
  const leftFixed = oldColumns.filter(item => item.fixed === 'left');
  const rightFixed = oldColumns.filter(item => item.fixed === 'right');
  return [...leftFixed, ...newColumns, ...rightFixed];
};
...
const askShouldImportData = ({ node, sourceRef }) => {
  useModal().confirm({
    ...
    exec() {
      ...
      if (sourceRef.value.data?.data) {
        node.props.data = [...sourceRef.value.data?.data];
      }
    },
    cancel() {
...
const extraHandlerMap = {
  VxeGrid: ({ node, sourceRef, schemaId, pageSchema }) => {
