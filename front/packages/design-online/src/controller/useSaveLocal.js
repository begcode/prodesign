...
import { VITE_ORIGIN } from '@/common/js/environments';
...
const confirmSaveLocal = async () => {
  ...
  if (savePage.error) {
    Modal.error({ title: savePage.error.message, status: 'error', duration: '5', top: 60 });
    return;
  }

  const errorMsg = savePage.data.reason ? `保存文件到本地失败！失败原因：${savePage.data.reason}` : '保存文件到本地失败！';

  const message = savePage.data.isSuccess ? '保存文件到本地成功' : errorMsg;

  savePage.data.isSuccess && (pageState.isSaved = true);

  Modal.error({ title: message, status: 'error', duration: '5000', top: 60 });
};
...
const savePageLocal = async () => {
  ...
  if (fileExistRes.error) {
    Modal.error({ title: fileExistRes.error.message, status: 'error', duration: '5', top: 60 });
    return;
  }
