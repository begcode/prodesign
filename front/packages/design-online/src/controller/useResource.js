...
import { Builtin, addScript, addStyle, canvasDispatch } from '@/canvas';
...
const fetchBlockDetail = async blockName => {
  ...
  const block = (await http.get(`http://localhost:3290/material-center/api/block?label=${blockName}`))?.[0];
...
const initPageOrBlock = async () => {
  ...
  const pageInfo = resState.pageTree.find(page => page?.meta?.isHome) ||
    resState.pageTree.find(page => page.componentName === COMPONENT_NAME.Page && page?.meta?.group !== 'publicPages') || {
...
const fetchResource = async ({ isInit = true } = {}) => {
  ...
  const appData = await useHttp().get(`http://localhost:3290/app-center/v1/api/apps/schema/${id}`);
  ...
  const locales = Object.keys(appData.i18n).length ? Object.keys(appData.i18n).map(key => ({ lang: key, label: key })) : defaultLocales;
  resState.langs = {
    locales,
