...
import { CanvasContainer, CanvasFooter, selectNode, getNodePath, updateRect } from '@/canvas';
import { useProperties, useCanvas, useLayout, useResource, useHistory, useModal } from '@/controller';
import materials from '@/plugins/materials';
import { useHttp } from '@/http';
import { constants } from '@/utils';
import { isVsCodeEnv, isDevelopEnv } from '@/common/js/environments';
import * as ast from '@/common/js/ast';
  ...
  setup() {
    ...
    watch(
      ...
      ([isSaved, pageStatus, pageSchema], [oldIsSaved, _oldPageStatus, oldPageSchema]) => {
        if ([PAGE_STATUS.Guest, PAGE_STATUS.Occupy].includes(useLayout().layoutState.pageStatus.state) || !pageSchema?.componentName) {
          return;
        }
