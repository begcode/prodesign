  ...
  <div id="tiny-engine-nav-panel" :style="{ 'pointer-events': pluginState.pluginEvent }">
      ...
      >
        <div>
          <span class="item-icon">
            <Icon v-if="typeof iconComponents[item.id] === 'string'" :icon="iconComponents[item.id]" class="panel-icon"></Icon>
    ...
    <ul class="nav-panel-lists bottom">
      ...
      <li
        ...
        :key="index"
        :class="['list-item', { active: renderPanel === item.id, prev: state.prevIdex - 1 === index, 'first-item': index === 0 }]"
        :title="item.title"
        @click="clickMenu({ item, index })"
      >
      ...
      <!--      <li :key="state.bottomNavLists.length + 1" :class="['list-item']" :title="state.independence.title" @click="openAIRobot">-->
      <!--        <div>-->
      <!--          <span class="item-icon">-->
      <!--            <img class="chatgpt-icon" src="@/assets/lowcode/AI.png" alt="chatgpt" />-->
      <!--          </span>-->
      <!--        </div>-->
      <!--      </li>-->
    </ul>
...
import { useLayout, usePage } from '@/controller';
...
export default {
  ...
  emits: ['click', 'node-click', 'close', 'fixPanel'],
  setup(props, { emit }) {
