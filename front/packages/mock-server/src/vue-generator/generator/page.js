...
import { DEFAULT_COMPONENTS_MAP, BUILTIN_COMPONENT_NAME, JS_FUNCTION, JS_RESOURCE, JS_SLOT, TINY_ICON } from '../constant';
...
const isEmptySlot = (componentName, children) => componentName === BUILTIN_COMPONENT_NAME.TEMPLATE && !(children?.length || children?.type);
...
const handleLiteralBinding = ({ key, item, attrsArr, description, state }) => {
  ...
  if (typeof item === 'object') {
    ...
    const canotBind =
      description.internalTypes.has(JS_FUNCTION) || description.internalTypes.has(JS_RESOURCE) || description.internalTypes.has(JS_SLOT);
...
function generateTemplate(schema, state, description, isRootNode = false) {
  ...
  if (loop) {
    const loopData = loop.type ? loop.value.replace(/this\.(props\.)?/g, '') : JSON.stringify(loop).replace(/"/g, '&quot;');
...
const generateVueCode = ({ schema, name, type, componentsMap }) => {
  ...
  const resourceStatement = usedResource.length ? `const { ${usedResource.join(',')} } = wrap(function() { return this })()` : '';
...
const generateCode = ({ pageInfo, componentsMap = [], blocksData = [] }) => {
  ...
  allComponents.forEach(item => !allComponentsMap.has(item.componentName) && allComponentsMap.set(item.componentName, item));
